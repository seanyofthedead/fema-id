...
| `config/` | **`mapping_rules.yaml`, `variance_trigger.yaml`, `risk_questions.yaml`, `schema_map.webifmis.yaml`, `schema_map.fims.yaml`, `code_bridge.yaml`** + a README; seeded into `config.*` (PLT-05) |
| `tests/` | `test_parity_synthetic.py` (both backends), `test_backend_parity.py`, `test_delta_roundtrip.py`, **`test_config_split.py`** — **51 tests**, no network |
| `pyproject.toml` | wheel metadata; `pandas` / `spark` / `dev` extras |
| still to come | `tools/make_packet.py`, `resources/`, `app/`, `notebooks/`, tasks 9–11 |
...
**The config split (done 2026-09-17).** `config/` now holds the pilot's runtime configuration in the shape §3's tree specifies, and the engine reads either that directory or the monolithic `rules.yaml` through one code path — so the split is a deployment choice, not a fork. Four things are worth stating:

- **Several files are deliberately empty, and the tests hold them that way.** There is no 20-program taxonomy, no alias map and no WebIFMIS source column names, because inventing them before the data walkthrough would be fabrication that is indistinguishable from real configuration once committed. With an empty taxonomy the engine maps nothing and every code lands in the exception queue with its spend excluded — the correct behaviour for "no rules yet", and loud rather than silent. Each file names the `SME-` question that fills it.
- **The split is proved equivalent rather than duplicated.** `test_config_split.py` refiles `rules.yaml` into the directory layout at runtime and asserts an identical `EngineConfig` *and* identical reportable tables on both backends. Committing a second copy of the demo taxonomy would have drifted; this cannot.
- **Configuration validates at seed time** (`EngineConfig.raise_for_problems()`): unknown trigger measure, threshold at or below zero, confidence outside 0–1, duplicate program / sub-program / rule / question IDs, a rule pointing at a sub-program no program declares, a quantitative question with no data binding. These are the failures that would otherwise surface as a wrong number inside a risk assessment rather than as a deploy error.
- **The PRA instrument became config** (`DEC-34`), on the DEC-31 pattern: the fixture's instrument now lives in `rules.yaml` and is read by both the generator and the engine, so `risk_question.csv` and the engine's bindings cannot drift; `config/risk_questions.yaml` is the pilot's copy, labelled a placeholder. That is what makes `AC-07` a file swap rather than a code change.
...
3. Here: ~~port tasks 3–8 with a pandas path; make the parity test pass against `data/synthetic/*.csv`; add the Spark adapter and re-run the same parity test locally; split `config/*.yaml`~~ **done 2026-09-17 (§3b)**; write `tools/make_packet.py`; author packets `PIIA-02` through `PIIA-05` (§3a).
