...
| FLOW-05 crawler holdout reveal | ✅ | 2026-07-09 | **Built into `leavebehind/fema-demo.html`** (via `template.html`, regenerated byte-stable): screen 3 *Inference test* tab — shown/not-shown split panels (train = FY2022–25, 396 mapping rows; holdout = FY2026, 105 codes, labels sealed), animated mining ticker, blind proposals with support-based confidence (99 codes at 0.94, auto ≥ 0.85; 6 no-history codes → no proposal → human-review routing into the existing exception queue), then an in-session **Reveal** scoring against the held-out FY2026 mapping: computed headline "99 of 99 auto-grouped codes match · 6 routed to review · 0 incorrect auto-classifications". Inference (`f5Infer`) and scoring (`f5Score`) are separated code paths; split constant `F5_HOLDOUT_FY` re-splits reproducibly; `answer_key.*` unread/un-embedded (build assertion passes); runs logged to the audit trail; presenter honesty caveat added to `DEMO_SCRIPT.md`. Verified in-browser: full sequence with no reload, deterministic across re-runs, all 10 screens regress clean, parity 740/740, zero network, no storage. DEC-30. |
