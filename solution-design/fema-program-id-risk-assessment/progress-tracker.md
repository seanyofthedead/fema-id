...
## 1e. Feedback-review planning artifacts (2026-07-11)

| Artifact | Status | Last updated | Notes |
|---|---|---|---|
| `source/Data_Dashboard_Presentation_otter.ai.REDACTED.txt` | ✅ | 2026-07-11 | Otter.ai auto-transcript of the team feedback walkthrough of `fema-demo.html` (generic "Speaker N" labels, approximate phrasing) — source-of-truth for the update analysis. Committed as a **role-redacted** copy (personal names → bracketed role labels; wording otherwise verbatim) per Sean's call; the unredacted original stays outside the repo |
...
| `REQ-` (feedback review, 2026-07-11) | 027…035 | Real-taxonomy, TAFS/fund-code, disbursement-type, disaster-indicator, count-trigger, status-wording, historical-RA ingestion (future), 3-year cycle, region drill-down (future). Defined in `updates/FEEDBACK_UPDATE_ANALYSIS.md`; file 02 unmodified |
| `ASSUMP-` (feedback review, 2026-07-11) | 20, 21, 22, 23, 24 | WebFMIS field stand-ins, count-rule parameters, Oct-1 modernization go-live, 5-program taxonomy stand-in, historical-RA data client-owned. Defined in `updates/FEEDBACK_UPDATE_ANALYSIS.md`; file 03 unmodified |
| `SME-` (feedback review, 2026-07-11) | 27, 28, 29, 30 | WebFMIS field names/formats, exact 2024 count-trigger rule, 3-year cycle + historical-RA terms, program-list confirmation. Defined in `updates/FEEDBACK_UPDATE_ANALYSIS.md`; file 13 unmodified |
...
- **Funded pilot (file 18):** tasks 3–8 of the FEMADex pipeline are ported to `src/fema_piia` on **both the pandas and Spark paths**, and the `PLT-13` parity gate is green on each — `pytest tests/` reproduces 4,014 committed values across `program_mapping`, `spend_summary`, `fiscal_year_spend_summary` and `risk_response` from `transaction.csv` + `rules.yaml`, then re-checks them after a Delta write/read round trip. 51 tests, no network; the Spark and Delta tests skip cleanly where neither is installed (file 18 §3b). The `config/` split is done and validated, with the taxonomy deliberately empty pending `SME-02`/`SME-04`. Acceptance criteria `AC-01…13` drafted for FEMA (`deliverables/ACCEPTANCE_CRITERIA.md`, due Oct 1–2). Next: `tools/make_packet.py` and packets `PIIA-02`–`PIIA-05`.
