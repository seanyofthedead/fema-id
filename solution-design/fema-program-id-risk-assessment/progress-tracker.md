...
| Artifact | Status | Last updated | Notes |
|---|---|---|---|
| `source/Data_Dashboard_Presentation_otter.ai.txt` | ⬜ local only | 2026-07-11 | Otter.ai auto-transcript of the team feedback walkthrough of `fema-demo.html` (generic "Speaker N" labels, approximate phrasing) — source-of-truth for the update analysis. **Placed in `source/` locally but deliberately NOT committed:** it names real stakeholders and is an internal meeting record; committing it to this remote is a provenance recommendation pending Sean's sensitivity review (see `updates/FEEDBACK_UPDATE_ANALYSIS.md` header) |
