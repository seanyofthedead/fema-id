...
- **Funded pilot (file 18):** tasks 3–8 of the FEMADex pipeline are ported to `src/fema_piia` on **both the pandas and Spark paths**, and the `PLT-13` parity gate is green on each — `pytest tests/` reproduces 4,014 committed values across `program_mapping`, `spend_summary`, `fiscal_year_spend_summary` and `risk_response` from `transaction.csv` + `rules.yaml`, then re-checks them after a Delta write/read round trip. 39 tests, no network; the Spark and Delta tests skip cleanly where neither is installed (file 18 §3b). Next: the `config/*.yaml` split, `tools/make_packet.py`, and packets `PIIA-02`–`PIIA-05`.
