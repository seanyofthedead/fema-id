...
def build_dataset(cfg: dict, anchors: dict) -> tuple[dict[str, bytes], dict]:
    ...
    inferred_subs = {"SUB-S05-A", "SUB-S06-A", "SUB-S11-C"}   # texture: still 'inferred'
    for prog in programs:
        fund = prog["fund_segment"]
        for sub in prog["sub_programs"]:
            sid = sub["sub_program_id"]
            if prog["mapping_style"] == "event_driven":
                seg = prog["program_segments"][0]
                expr = (f"fund_segment == '{fund}' and program_segment == '{seg}' "
    ...
    for prog in programs:
        ...
        weights = [envelope[c["event"]] for c in clist]
        for fy in fys:
    ...
    t_prog = Table("program", ["program_id", "program_name", "assistance_listing", "data_watermark"])
    ...
    for prog in programs:
        t_prog.add(program_id=prog["program_id"], program_name=prog["program_name"],
                   assistance_listing=prog["assistance_listing"], data_watermark=wm)
        for sub in prog["sub_programs"]:
    ...
    t_code = Table("financial_code", ["code", "sub_program_id", "fund_segment", "program_segment",
                                      "event_segment", "data_watermark"])
    for c in codes:
        t_code.add(code=c["code"], sub_program_id=c["sub_program_id"], fund_segment=c["fund_segment"],
                   program_segment=c["program_segment"], event_segment=c["event"], data_watermark=wm)
    for e in exc:
        t_code.add(code=e["code"], sub_program_id="", fund_segment=e["fund_segment"],
                   program_segment=e["program_segment"], event_segment=e["event"], data_watermark=wm)
    ...
    t_txn = Table("transaction", ["txn_id", "raw_code", "code", "disaster_number", "fiscal_year",
                                  "disbursement_amount", "disbursement_date", "data_watermark"])
    txn_no = 0
    n_dirty = 0
    n_alias = 0
    ...
    def emit_txns(code: str, event: int, fy: int, total: int, n: int, alias_ok: bool) -> None:
        ...
        for part in parts:
            ...
            if alias_ok and fy <= 2023 and rng.random() < 0.5:
                ...
                n_dirty += 1
            t_txn.add(txn_id=f"TXN-{txn_no:06d}", raw_code=raw, code=code, disaster_number=event,
                      fiscal_year=fy, disbursement_amount=money(part),
    ...
    for prog in programs:
        ...
        for fy in fys:
            ...
            n_prog = max(6, min(48, prog_total // cents(12_000_000) + 6))
            shares = [code_fy[(c["code"], fy)] for c in clist]
            tot_share = sum(shares) or 1
            for c in clist:
                amt = code_fy[(c["code"], fy)]
                n = max(1, round(n_prog * amt / tot_share))
                emit_txns(c["code"], c["event"], fy, amt, n, alias_ok=c["code"] in alias_rev)

    for e in exc:   # FY2026-only unmapped codes (REQ-003 plants)
    ...
    for prog in programs:
        ...
        for fy in fys:
            for ev in events:
                ...
                t_ss.add(summary_id=f"SS-{pid}-{fy}-{ev}", program_id=pid, fiscal_year=fy,
                         disaster_number=ev, total_disbursement=money(cur),
    ...
    t_fss = Table("fiscal_year_spend_summary",
                  ["summary_id", "program_id", "fiscal_year", "total_disbursement",
                   "prior_year_disbursement", "yoy_pct_change", "trigger_flag",
    ...
    for prog in programs:
        ...
        for fy in fys:
            ...
            ev_totals = [pe_total.get((pid, fy, ev), 0)
                         for ev in sorted({c["event"] for c in prog_codes[pid]})]
            ...
            rec = {
                ...
                "prior_year_disbursement": money(prior) if fy != fys[0] else "",
                "yoy_pct_change": "" if yoy is None else f"{yoy:.1f}",
                "trigger_flag": str(fires(yoy, prior)).lower(),
    ...
    q_rows = [
        ...
        ("Q3", "Does YoY change breach the comprehensive-assessment threshold? (illustrative placeholder, ASSUMP-04)",
         "quantitative", "true", "fiscal_year_spend_summary.trigger_flag"),
    ...
    for prog in programs:
        ...
        answers = {
            ...
            "Q8": (f"FY{pra_fy - 1}: comprehensive assessment triggered"
                   ...
                   else f"FY{pra_fy - 1}: preliminary only, no trigger"),
        }
    ...
    for c in codes:
        t_ak.add(record_type="code_mapping", raw_or_alias_code="", canonical_code=c["code"],
                 ...
                 event_segment=c["event"], disaster_number=c["event"],
...
def build_answer_key_md(cfg: dict, stats: dict) -> str:
    ...
    for prog in stats["programs"]:
        ...
        lines.append(f"| {prog['program_id']} | {', '.join('DR-' + str(e) for e in evs)} |")
    lines += [
        "",
    ...
    lines += [
        f"| crosses +{stats['threshold']:.0f}% (trigger) | {', '.join(ups)} |",
        f"| crosses −{stats['threshold']:.0f}% (trigger) | {', '.join(downs)} |",
        f"| within ±{stats['threshold']:.0f}% (no trigger) | {', '.join(within)} |",
        "",
...
def run_self_checks(files: dict[str, bytes], files2: dict[str, bytes], stats: dict,
    ...
    results.append(("2 parent with exactly 3 sub-programs (REQ-004)", "PROG-PA" in exactly3,
    ...
    # 5. variance coverage at configured threshold (REQ-010, ASSUMP-03)
    ups = downs = within = 0
    for prog in stats["programs"]:
        y = stats["fss_lookup"][(prog["program_id"], fys[-1])]["_yoy"]
        if y is None:
        ...
        if y >= threshold:
            ...
            within += 1
    ok5 = ups >= 2 and downs >= 1 and within >= 2
    results.append((f"5 variance coverage at ±{threshold:.0f}%", ok5,
                    f"+crossers={ups}, -crossers={downs}, within={within}"))
    ...
    total_syn = sum(stats["fss_lookup"][(p["program_id"], fy)]["_cur"]
                    for p in stats["programs"] for fy in fys)
    ...
    results.append(("8 magnitudes within obligation-anchor band (ASSUMP-10)", ok8,
