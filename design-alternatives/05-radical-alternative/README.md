...
**Pan/zoom on one continuous canvas is the only navigation.** Vertical position = program, horizontal position = time; overview → lane → fiscal-year interval → underlying records is a zoom path, not a page path. Controls: zoom/pan buttons, mouse wheel, keyboard (+ / − / 0 to zoom and reset, Shift+Left/Right to pan), an FY scrubber, and lane-focus mode (expands a lane with real dual y-axes: dollars left, transaction counts right). Everything clickable on the canvas opens a persistent evidence drawer that survives further scrubbing — investigation context is never thrown away. A drag across the canvas (or two selects) sets the two-year comparison window. Escape closes overlays.

**Keyboard model (roving tabindex).** The canvas is exactly **one** tab stop — keyboard users are never forced through 100+ sequential points. Arrow keys move a roving focus among the canvas's interactive elements: Left/Right walk a lane's points, flares, and milestones in time order; Up/Down jump between lanes (and up to the context track), landing on the nearest moment in time; Enter/Space activates. Every move is announced through an `aria-live` region using the element's full descriptive label (program, fiscal year, exact values, verdict), and the roving stop follows mouse activation so pointer and keyboard users share one focus model. The detail ledger below remains the linear, screen-reader-equivalent surface for the whole canvas; all data tables carry proper header scopes.
